...
const resetAuth = () => {
    ...
    user_tokenProperties.deleteProperty('dscc.slToken');
};

const isAuthValid = (): boolean => {
    const usernameAndToken = loadCurrentUsernameAndToken();
    const result = usernameAndToken.username && usernameAndToken.token && validateCredentials(usernameAndToken.username, usernameAndToken.token);
    return !!result;
};
...
const storeUsernameAndToken = (username: string, token: string, slToken: string) => {
    PropertiesService
        .getUserProperties()
